...
this._dListeners.registerError=_reactNative.DeviceEventEmitter.addListener('RNZeroconfRegisterError',function(service){
_this2.emit('error',service.error);
});

this._dListeners.unregisterError=_reactNative.DeviceEventEmitter.addListener('RNZeroconfUnregisterError',function(service){
if(!service||!service.name){return;}
_this2.emit('error',service.error);
});

this._dListeners.registered=_reactNative.DeviceEventEmitter.addListener('RNZeroconfRegistered',function(service){
if(!service||!service.name){return;}
_this2.emit('registered',service);
});

this._dListeners.unregistered=_reactNative.DeviceEventEmitter.addListener('RNZeroconfUnregistered',function(service){
if(!service||!service.name){return;}
_this2.emit('unregistered',service);
});
